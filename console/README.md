...
界面设计语言「桌面」：控制台长成设备周围的那张桌子——深墨桌面（机身边框同族暖黑）上摆**一张纸**（整页一张、印刷栏线分区，不做卡片堆叠），取色自 7.5″ 四色墨水屏（纸白/墨黑/屏红/屏黄：红只做印章式强调与数据色，黄做焦点环）；标题走衬线印刷字配等宽英文副标，地址/日志/JSON 走等宽字，设备画布以真机边框形态出场（设计系统集中在 `css/style.css`，类名契约与 `js/` 解耦）。

氛围层「潮汐」：墨带顶栏下沿 10s 呼吸海平线、在线徽标灯珠脉动、设备在线时设备框亮屏黄晕光（岛灯）、推送成功盖「已上屏」屏红竖印（失败墨黑「未上屏」）、工作台右缘竖排题词、空布局鲸影屏中缓游、页首约每分钟一道鲸影游过（顶栏「鲸影」按钮 开/频/关 三态）。全部动效尊重系统「减少动态」设置（`prefers-reduced-motion` 下静止）。
...
两版功能一致；改版后若样式未更新，强刷一次（Ctrl+F5）——静态资源引用已带 `?v=` 版本参数。**页脚带 UI 版本号戳**（如 `UI v20260926k`），反馈界面问题时连同截图给出即可定位版本。
...
- **连接瞬间板子可能重启一次**：Chrome 打开串口时默认拉高 DTR/RTS，ESP32 的串口自动复位电路（与烧录复位同通路）可能视为复位脉冲——板子约 2s 自愈，页面检测到启动横幅会提示并自动补发状态探测；连接成功后已立即清 DTR/RTS 把脉冲窗口压到最短；
