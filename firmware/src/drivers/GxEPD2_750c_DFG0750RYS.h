// ============================================================
// 驱动层 · 7.5" 四色墨水屏 880×528（DFG0750RYS677F71HP-D1 手册口径）实验驱动
//
// 【B1 实测结论 2026-09-22：不适用本批屏，仅留档】到货屏实为 800×480——
// 本驱动（880×528）下屏仅显示上部 480 行（底部文字/角标被裁）即证；随屏
// 手册（标称 880×528）与实物不符，疑卖家错配文档。若后续批次屏确为
// DFG0750RYS677F71HP-D1（FPC 丝印可核），在 epaper.h 换 using 行启用本类。
//
// 由 GxEPD2 1.6.9 的 epd4c/GxEPD2_750c_GDEM075F52 原样派生（同 JD79665AA
// 家族控制器，命令表与屏手册 §7 逐条吻合），仅两处差异：
//   WIDTH 800→880、HEIGHT 480→528（_InitDisplay 的 TRES 0x61 随之自适应）。
// 其余初始化序列保持 GxEPD2 原样（该序列已在 Waveshare/佳显同族屏量产验证）。
// 实测记录：本批 800×480 屏接受 880×528 寻址且刷新正常（21.8s，画面上部
// 正确显示、超出物理行不显示，无损坏）——也反证控制器为 960×680 上限同族。
// ============================================================
...
#ifndef _GxEPD2_750c_DFG0750RYS_H_
#define _GxEPD2_750c_DFG0750RYS_H_

#include <GxEPD2_EPD.h>

class GxEPD2_750c_DFG0750RYS : public GxEPD2_EPD
{
  public:
    // attributes
    ...
    // constructor
    GxEPD2_750c_DFG0750RYS(int16_t cs, int16_t dc, int16_t rst, int16_t busy);
    // methods (virtual)
