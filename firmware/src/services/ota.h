...
// 安全性：https 走根证书校验（certs.h）；提供 md5 时启用完整性校验
//       （Update.setMD5，防"长度正确的损坏/篡改镜像"）；
//       下载中断/校验失败只 abort，当前固件不受影响；
//       写入期间勿断电（闪存写入窗口，约数秒）。
...
// 下载并刷入备用分区，成功后重启；md5 可选（32 位十六进制，Release 附件 .md5）
void fromUrl(const char* url, const char* md5 = nullptr);
